...
<div class="container-fluid content-inner py-3">
    ...
    <div class="card">
        ...
        <div class="card-body">
                        ...
                        @forelse($data as $index => $item)
                            <tr>
                                ...
                                <td class="text-center">
                                    ...
                                    <form action="{{ route('harga-cpo.destroy', $item->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                              ...
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title">Edit Harga CPO</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                  </div>
                                  <div class="modal-body">
                                    <form action="{{ route('harga-cpo.update', $item->id) }}" method="POST">
                                      @csrf
                                      <div class="mb-3">
                                        <label>Tanggal:</label>
            ...
            {{-- Grafik Harga --}}
            <div class="mt-5">
                <h5 class="text-center mb-3">Grafik Harga CPO Mingguan</h5>
                <canvas id="chartHargaCpo" height="120"></canvas>
            </div>
...
<div class="modal fade" id="tambahHargaModal" tabindex="-1" aria-labelledby="tambahHargaModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ route('harga-cpo.store') }}" method="POST">
          @csrf
          <div class="mb-3">
            <label>Tanggal:</label>
...
<script>
const ctx = document.getElementById('chartHargaCpo');
const chart = new Chart(ctx, {
    type: 'line',
    data: {
        labels: {!! json_encode($data->pluck('tanggal')->map(fn($d) => \Carbon\Carbon::parse($d)->format('d M Y'))) !!},
        datasets: [{
